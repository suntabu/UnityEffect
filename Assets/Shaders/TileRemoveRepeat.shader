...
//                float2 uv1 = iuv.xy;
//                float2 uv2 = iuv.yx;
                float2 uv3 = float2(fuv.x, -fuv.y);
                float2 uv4 = float2(fuv.y, -fuv.x);
                float2 uv5 = float2(-fuv.x, fuv.y);
                float2 uv6 = float2(fuv.y, fuv.x);
                float x = sin(iuv.x * 1.5707);
                float y = sin(iuv.y * 1.5707);
//                float4 gap = sin(iuv.x * 3.14);
                float2 gap = abs(float2(x,y));
//                gap = smoothstep(_BlendRatio, 1.0 - _BlendRatio, gap);
                
                return lerp(lerp(tex2D(tex, uv3), tex2D(tex, uv4), gap.x),
                                lerp(tex2D(tex, uv5), tex2D(tex, uv6), gap.x), gap.y);
            }
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 brush = texNoTileTech2(_BrushTex, i.uv2 );
				brush.a = col.a;
